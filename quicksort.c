...
/*   Updated: 2019/07/12 03:32:26 by twight           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		move_ab(t_cont *cont)
{
	...
	if (A_FIRST && A_SECOND) // to be removed, if applicable.
	...
	while (!is_ordered(cont))
	{
		...
		while (1)
		{
			...
			while (tmp && tmp->value >= cont->pivot)
				tmp = tmp->next;
			if (!tmp)
				break ; // To find out how to break cycle in understandable way
			while (A_FIRST_VALUE < cont->pivot)
				move_ab(cont);
			if (A_SECOND && A_FIRST_VALUE > A_SECOND_VALUE && A_SECOND_VALUE > cont->pivot)
				sa(cont);
